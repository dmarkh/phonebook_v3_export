...
class Display {
	...
	setGDMLParameters( par = {}) {
		this.gdml_parameters = { ...this.gdml_parameters, ...par };
	}

	async updateGDMLParameters( par = {} ) {
		this.setGDMLParameters( par );
		if ( this.GDMLimporter ) {
			this.GDMLimporter.updateParameters( this.gdml_parameters );
			await this.add_detector();
        	this.render();
		}
	}

	reset_gdml( onStart = false, onProgress = false, onFinish = false ) {
		this.GDMLimporter = new GDML({});
		this.GDMLimporter.cbOnStart = onStart;
		this.GDMLimporter.cbOnProgress = onProgress;
		this.GDMLimporter.cbOnFinish = onFinish;
		this.GDMLimporter.updateParameters( this.gdml_parameters );
	}
	...
	async init() {
		...
		this.canvas = document.getElementById( this.parameters.canvas_id );

		if ( this.is_initialized ) {
			// update renderer
			this.camera_perspective.aspect = this.canvas.width / this.canvas.height;
			this.camera_perspective.updateProjectionMatrix();
			this.camera_ortho.left  = -this.canvas.width;
			this.camera_ortho.right =  this.canvas.width;
			this.camera_ortho.top   =  this.canvas.height;
			this.camera_ortho.bottom =-this.canvas.height;
		...
		this.camera_perspective = new THREE.PerspectiveCamera( this.fov, this.canvas.width / this.canvas.height, 10, 200000 );
		...
		this.renderer = new THREE.WebGLRenderer({ antialias: true,  preserveDrawingBuffer: true, alpha: true,
								logarithmicDepthBuffer: false, canvas: this.canvas });
...
*/
	...
	update_infobox( evt ) {
		if ( evt ) {
			const e = document.getElementById("display-infobox-content");
			if ( evt.EVENT.runstats ) {
				e.innerHTML = evt.EVENT.runstats.map( v => '<div style="font-family: inherit !important;">' + v + '</div>' ).join('');
			} else {
				const ihtml = [];
				if ( evt.EVENT.runid && evt.EVENT.evtid ) {
					ihtml.push( '<div> Run: ' + evt.EVENT.runid + ', Event: ' + evt.EVENT.evtid + '</div>' );
				}
				if ( evt.EVENT.timeStr ) {
					ihtml.push( '<div>' + evt.EVENT.timeStr + '</div>' );
				} else if ( evt.EVENT.time ) {
					ihtml.push( '<div>' + evt.EVENT.time + '</div>' );
				}
				if ( evt.EVENT.type && evt.EVENT['s_nn'] ) {
					ihtml.push( '<div>' + evt.EVENT.type + ' @ √' + evt.EVENT['s_nn'] + ' GeV' + '</div>' );
				}
				if ( ihtml.length ) {
					e.innerHTML = ihtml.join("\n");
				}
			}
			const ev = document.getElementById("display-infobox");
			if ( ev ) {
...
*/
	...
	async load_event_as_file( url ) {
		...
		await this.EVENTimporter.parse();
		this.update_infobox( this.EVENTimporter.evt );
		return true;
	}

	async load_event_as_text( text ) {
		await this.EVENTimporter.parse( text );
		this.update_infobox( this.EVENTimporter.evt );
		return true;
	}
