...
import { getInstitutionFields, getInstitutionFieldgroups, updateInstitutionFields } from '../utils/pnb-api.js';
...
const fetchData = async() => {
	let names = [
		...
		{ 'name': 'privacy', 'editable': 1, 'options': [ ['public','PUBLIC'], ['users_auth','USER'], ['users_admin','ADMIN'] ] },
		{ 'name': 'always_latest', 'editable': 1, 'options': [ ['y','YES'], ['n','NO'] ] }
		];
	...
	(names.find( n => n.name == 'group'))['options'] = fieldgroup_options;
...
const updateRecord = async () => {
    ...
    let rc = await updateInstitutionFields( data );

	await sleep(1000);
	router.goto('/institution-field/' + meta.params.id + '/view');
	pleaseWait = false;
}
...
{:then data}
    <div style="text-align: center;" class="mdc-typography--headline4">INSTITUTION FIELD DESCRIPTOR: {meta.params.id}</div>
