...
const saveAuthorListFile = () => {
	...
	let filename = unix_ts ? ( "author-list-iop-" + unix_ts + ".txt" ) : ( "author-list-iop-" + ( new Date().toISOString().slice(0, 10) ) + ".txt" );
    FileSaver.saveAs(blob, "author-list-iop-" + filename );
}
...
const getAuthorListIOP = async () => {
	...
	let field, ifield, inst_id, f_leave_date = find_field_id( mfields, 'date_leave'),
        ...
        for ( let i in members ) {
            ...
            if ( !field || ( members[i].status !== 'active' && strtotime( members[i]['status_change_date'] ) < now ) ) {
				continue;
			}

            if (
				field[ f_leave_date ] !== undefined &&
                field[ f_leave_date ] !== '0000-00-00 00:00:00' &&
                field[ f_leave_date ] !== '0000-00-00' &&
