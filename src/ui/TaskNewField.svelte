...
import { getTaskFields, createTaskField } from '../utils/pnb-api.js';
...
const updateRecord = async () => {
	pleaseWait = 'ADDING NEW TASK FIELD, PLEASE WAIT';
	...
	let rc = await createTaskField( data );

	await sleep(1000);
	router.goto('/task-field/' + rc.id + '/view');
	pleaseWait = false;
}
...
{:then data}
    <div style="text-align: center;" class="mdc-typography--headline4">NEW TASK FIELD DESCRIPTOR</div>
