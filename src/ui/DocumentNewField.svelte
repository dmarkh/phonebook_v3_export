...
import { getDocumentFields, createDocumentField } from '../utils/pnb-api.js';
...
const updateRecord = async () => {
	pleaseWait = 'ADDING NEW DOCUMENT FIELD, PLEASE WAIT';
	...
	let rc = await createDocumentField( data );

	await sleep(1000);
	router.goto('/document-field/' + rc.id + '/view');
	pleaseWait = false;
}
...
{:then data}
    <div style="text-align: center;" class="mdc-typography--headline4">NEW DOCUMENT FIELD DESCRIPTOR</div>
