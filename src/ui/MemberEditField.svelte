...
import { getMemberFields, getMemberFieldgroups, updateMemberFields } from '../utils/pnb-api.js';
...
const updateRecord = async () => {
	...
	let rc = await updateMemberFields( data );

	await sleep(1000);
	router.goto( '/member-field/' + meta.params.id + '/view' );
	pleaseWait = false;
}
...
{:then data}
    <div style="text-align: center;" class="mdc-typography--headline4">MEMBER FIELD DESCRIPTOR: {meta.params.id}</div>
