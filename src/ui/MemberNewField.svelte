...
import { getMemberFields, getMemberFieldgroups, createMemberField } from '../utils/pnb-api.js';
...
const fetchData = async() => {
	...
	let fieldgroups = await getMemberFieldgroups();
...
const updateRecord = async () => {
	pleaseWait = 'ADDING NEW MEMBER FIELD, PLEASE WAIT';
	...
	let rc = await createMemberField( data );

	await sleep(1000);
	router.goto('/member-field/' + rc.id + '/view');
	pleaseWait = false;
}
...
{:then data}
    <div style="text-align: center;" class="mdc-typography--headline4">NEW MEMBER FIELD DESCRIPTOR</div>
