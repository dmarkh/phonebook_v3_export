...
const add_display_field = (idx) => {
	display_fields = [ ...display_fields, "title" ];
}
...
let init = async () => {
	...
	documents = await getDocuments('full',0,1000000);
    ...
    for ( const item of converted_documents ) {
        if ( item.group_id  ) { item.group = get_group_name( item.group_id );   }
        if ( item.author_id ) { item.owner = get_member_name( item.author_id ); }
    }
