...
const filterItemsQuick = () => {
    if ( $quickSearch ) {
        filtered_groups = gdata.tgroups.filter( item => {
			...
			if ( group ) {
				if ( group.name.toLowerCase().includes( $quickSearch.toLowerCase() ) ) { return true; }
				if ( group.desc.toLowerCase().includes( $quickSearch.toLowerCase() ) ) { return true; }
			}
            return false;
        });
    } else {
...
{:else}
	...
	{:then group_data}
    	...
    	<div style="text-align: center; margin-top: 2vmin;">
	        <Textfield variant="outlined" bind:value={$quickSearch} label="Search My Groups">
			...
			{#each filtered_groups as grp (grp.group_id)}
				...
				<Row data-entry-id="group">
			        ...
			        <Cell style="text-align: center;">
				        <Button color="primary" on:click={() => { leave_group( grp.group_id ); }} variant="raised">
				          <Icon class="material-icons">logout</Icon>
				          <ButtonLabel>LEAVE GROUP</ButtonLabel>
				        </Button>
					</Cell>
				</Row>
				{/if}
  ...
  <Pagination slot="paginate">
    ...
    >
		...
		<br /><br />
		<div style="text-align: center;" class="mdc-typography--headline6">SEARCH AND JOIN GROUPS</div>

    	<div style="text-align: center; margin-top: 2vmin;">
	        <Textfield variant="outlined" bind:value={$groupSearch} label="Search All Groups">
            	<Icon class="material-icons" slot="trailingIcon">search</Icon>
    	    </Textfield>
	    </div>

		{#if filtered_new_groups.length}
		<br />
		<DataTable table$aria-label="All Groups Data" style="width: 100%;">
		    <Head>
        		<Row>
		    ...
		    <Body>
			{#each filtered_new_groups as grp (grp.id)}
				<Row data-entry-id="group">
			        <Cell style="text-align: center;">{ grp.name }</Cell>
			        <Cell style="text-align: center;">{ grp.private }</Cell>
			        <Cell style="text-align: center;">
						{#if grp.private == 'no'}
					        <Button color="primary" on:click={() => { join_group( grp.id ); }} variant="raised">
					          <Icon class="material-icons">login</Icon>
					          <ButtonLabel>JOIN GROUP</ButtonLabel>
					        </Button>
						{/if}
					</Cell>
				</Row>
