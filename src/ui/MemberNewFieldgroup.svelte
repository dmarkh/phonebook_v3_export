...
import { getMemberFields, getMemberFieldgroups, createMemberFieldgroup } from '../utils/pnb-api.js';
...
const updateRecord = async () => {
	pleaseWait = 'ADDING NEW MEMBER FIELDGROUP, PLEASE WAIT';
	...
	let rc = await createMemberFieldgroup( data );

	await sleep(1000);
	router.goto('/member-fieldgroup/' + rc.id + '/view');
	pleaseWait = false;
}
...
{:then data}
    <div style="text-align: center;" class="mdc-typography--headline4">NEW MEMBER FIELDGROUP DESCRIPTOR</div>
