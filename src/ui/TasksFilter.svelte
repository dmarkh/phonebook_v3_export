...
let tasks = false,
	converted_tasks = false,
	filtered_tasks = false,
	fields = false,
...
let display_fields = JSON.parse(JSON.stringify( window.pnb['filter-tasks']['display-fields'] )),
	sort_fields = JSON.parse(JSON.stringify( window.pnb['filter-tasks']['sort-fields'] ));
...
const add_filter = (idx) => {
	filters = [ ...filters, { "field": "title", "op": "notempty", "value": "" } ];
}
...
const apply_filters = async () => {
	...
	showFiltered = true;
	filtered_tasks = [];
	for ( const i of converted_tasks ) {
...
{:then}
<div style="text-align: center;" class="mdc-typography--headline4">FILTER TASKS</div>
<Paper>
<p>FILTERS</p>
<div>
...
{:else}
	...
	<div style="text-align: center;" class="mdc-typography--headline4">FILTERED TASKS:</div>
...
>
    ...
    <Body>
    {#each filtered_tasks as item (item.id)}
