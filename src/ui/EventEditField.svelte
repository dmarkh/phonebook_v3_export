...
import { getEventFields, updateEventFields } from '../utils/pnb-api.js';
...
const updateRecord = async () => {
    ...
    let rc = await updateEventFields( data );

	await sleep(1000);
	router.goto('/event-field/' + meta.params.id + '/view');
	pleaseWait = false;
}
...
{:then data}
    <div style="text-align: center;" class="mdc-typography--headline4">EVENT FIELD DESCRIPTOR: {meta.params.id}</div>
