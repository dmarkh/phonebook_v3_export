...
const get_stats_data = async () => {
    ...
    for ( const v of groups ) {
        ...
        groupmap[ id ] = {};
        years.forEach( y => { groupmap[id][y] = { "fte-sum": 0 }; } );
    }
    ...
    for ( const v of atasks ) {
		...
		const grp = groupsbyid[v.group_id];
		if ( !tasksbyid[v.task_id] ) { continue; }
        for ( const y of years ) {
            if (
                ( !v.begin_time_year || v.begin_time_year <= y )
                && ( !v.end_time_year || v.end_time_year >= y )
            ) { groupmap[ grp.id ][ y ]['fte-sum'] += Number(v.fte); }
        }
    }

    let data = [];
    for ( const g of groups ) {
        let item = { id: g.id, name: g.name };
        for ( const year of years ) {
            item[year] = roundToOneDecimal( groupmap[ g.id ][ year ]['fte-sum'] );
        }
        data.push( item );
    }
...
<DataTable table$aria-label="Task Stats By Group" style="width: 100%;"
  sortable
  bind:sort
  bind:sortDirection
