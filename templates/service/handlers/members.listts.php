...
function members_listts_handler($params) {
  ...
  $query = 'SELECT * FROM `'.$db_name.'`.`members` WHERE `join_date` <= FROM_UNIXTIME('.$ts.')';
  $mem = $db->Query($query);
  ...
  $query = 'SELECT *, UNIX_TIMESTAMP(`value_to_date`) AS uvtd, UNIX_TIMESTAMP(`value_from_date`) AS uvfd FROM `'.$db_name.'`.`members_history` WHERE UNIX_TIMESTAMP(`date`) > '.$ts.' ORDER BY `date` DESC';
  ...
  $flt_compact = array( $mfldi['institution_id'], $mfldi['name_first'], $mfldi['name_initials'], $mfldi['name_last'], $mfldi['name_unicode'],
			...
			$mfldi['is_author'],$mfldi['is_expert'],$mfldi['is_shifter'], $mfldi['is_deceased'],
			$mfldi['inspire_id'], $mfldi['orcid_id'], $mfldi['gender'] );
