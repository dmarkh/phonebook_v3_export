...
function create_institutions_fields($data) {
	...
	$query = 'INSERT INTO `'.$db_name.'`.`institutions_fields` (`name_fixed`,`name_desc`,`weight`,`type`,`options`,`size_min`,`size_max`,`hint_short`,`hint_full`,`group`, `is_required`, `is_enabled`, `privacy`, `always_latest`)'
...
function create_members_fields($data) {
	...
	$query = 'INSERT INTO `'.$db_name.'`.`members_fields` (`name_fixed`,`name_desc`,`weight`,`type`,`options`,`size_min`,`size_max`,`hint_short`,`hint_full`,`group`, `is_required`, `is_enabled`, `privacy`, `always_latest`)'
		.' VALUES ("'.$db->Escape($data['name_fixed']).'", "'.$db->Escape($data['name_desc']).'", '.intval($data['weight'])
		.', "'.$db->Escape($data['type']).'", "'.$db->Escape($data['options']).'", '.intval($data['size_min']).', '.intval($data['size_max'])
		.', "'.$db->Escape($data['hint_short']).'", "'.$db->Escape($data['hint_full']).'", '.intval($data['group']).', "'.$db->Escape($data['is_required']).'", '
		.' "'.$db->Escape($data['is_enabled']).'", "'.$db->Escape($data['privacy']).'", "'.$db->Escape($data['always_latest']).'")';
...
function create_institutions_fields_groups($data) {
	...
	$query = 'INSERT INTO `'.$db_name.'`.`institutions_fields_groups` (`name_short`, `name_full`, `weight`,`is_enabled`) VALUES ("'
		.$db->Escape($data['name_short']).'", "'.$db->Escape($data['name_full'])
		.'", '.intval($data['weight']).', "'.$db->Escape($data['is_enabled']).'")';
	$db->Query($query);
	$id = $db->LastID();
	return [ 'id' => $id ];
}
