...
function search_members_by_field($keyword, $field_id) {
	...
	$query = 'SELECT DISTINCT(`members_id`), `value`, damlev(LOWER(`value`), LOWER("'.$keyword.'")) AS dist FROM `'.$db_name.'`.`members_data_strings` WHERE `members_fields_id` = '.intval($field_id).' AND damlev(LOWER(`value`),LOWER("'.$keyword.'")) <= 2 ORDER BY dist LIMIT 10';
...
function search_members($field_id, $keyword, $type, $autocomplete) {
	...
	switch ($type) {
		case 'combined':
			...
			$query = 'SELECT DISTINCT(`members_id`), `value`, damlev(LOWER(`value`), LOWER("'.$keyword.'")) AS dist FROM `'.$db_name.'`.`members_data_strings` WHERE damlev(LOWER(`value`),LOWER("'.$keyword.'")) <= 2 '.$field_limit.' ORDER BY dist LIMIT 5';
			$r = $db->Query($query);
			$res = array_merge($res, $r);
		...
		case 'damlev':
			if ($autocomplete == true) {
				$query = 'SELECT `members_id`, `value`, damlev(LOWER(`value`), LOWER("'.$keyword.'")) AS dist FROM `'.$db_name.'`.`members_data_strings` WHERE damlev(LOWER(`value`),LOWER("'.$keyword.'")) <= 2 '.$field_limit.' GROUP BY `value` ORDER BY dist LIMIT 10';
			} else {
				$query = 'SELECT DISTINCT(`members_id`), `value`, damlev(LOWER(`value`), LOWER("'.$keyword.'")) AS dist FROM `'.$db_name.'`.`members_data_strings` WHERE damlev(LOWER(`value`),LOWER("'.$keyword.'")) <= 2 '.$field_limit.' ORDER BY dist LIMIT 10';
			}
			$res = $db->Query($query);
			break;
		case 'soundex':
	...
	switch ($type) {
		case 'combined':
			$query = 'SELECT institutions_id, `value` FROM `'.$db_name.'`.`institutions_data_strings` WHERE `value` LIKE "%'.$keyword.'%" '.$field_limit.' GROUP BY `institutions_id` ORDER BY `institutions_id`';
	   		$r = $db->Query($query);
			$res = array_merge($res, $r);
			$query = 'SELECT DISTINCT(`institutions_id`), `value`, damlev(LOWER(`value`), LOWER("'.$keyword.'")) AS dist FROM `'.$db_name.'`.`institutions_data_strings` WHERE damlev(LOWER(`value`),LOWER("'.$keyword.'")) <= 2 '.$field_limit.' ORDER BY dist LIMIT 10';
    		$r = $db->Query($query);
			$res = array_merge($res, $r);
