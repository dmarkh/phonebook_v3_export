...
function email_member_get_fields() {
...
function check_email_handler($params) {
  $cnf =& ServiceConfig::Instance();
  $db =& ServiceDb::Instance('phonebook_api');
  $db_name = $cnf->Get('phonebook_api','database');
  ...
  $query = 'SELECT m.id as members_id FROM `'.$db_name.'`.`members` as m, `'.$db_name.'`.`members_data_strings` as mds'
    .' WHERE m.id = mds.members_id AND mds.members_fields_id = '.intval($email_fid).' AND mds.value = "'.$db->Escape($email).'"';
