...
# 
# Export current institution list in MS Excel format. 
#
# fields, sort - required options
#
# /institutions/excelx/fields:[1,2,3..N]/sort:[F1,F2]
#
# institutions: active
...
function institutions_excelx_handler($params) {
  ...
  foreach($inst as $k => $m) {
	...
	foreach($req_fields as $k2 => $fld) {
		$val = '';
		if (isset($inst_fields[$m['id']][$fld])) {
			$val = $inst_fields[$m['id']][$fld];
		}
  ...
  header('Content-Disposition: attachment;filename="institutions-'.date('Y-m-d').'-'.time().'.xlsx"');
